...
module.exports = class Util {
        ...
        try {
            ...
            if (funny === 'first') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:Xerox_Antinuke:1255434662319226900> Antinuke \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'second') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:Xerox_Moderation:1255448773467967488> Moderation \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'third') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:Xerox_Utility:1255448776387067904> Utility \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'fourth') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:Xerox_Welcomer:1255448778941665333> Welcomer \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'fifth') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:Xerox_Voice:1255448787560955934> Voice \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'seven') {
                _commands = this.client.commands
                    ...
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:Xerox_Logging:1255448784222027918> Logging \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'six') {
                _commands = this.client.commands
                    .filter((x) => x.category && x.category === 'role')
                    .map((x) => `\`${x.name}\``)
                embed.addField(
                    `**<:Xerox_Roles:1255448781705576469> Roles \`[${_commands.length}]\`**`,
                    _commands.sort().join(', ')
                )
                interaction
                    .update({
            ...
            if (funny === 'eight') {
                ...
                embed.addField(
                    `**<:Xerox_Automod:1255448810566455387> Automod \`[${cmd.length}]\`**`,
                    cmd.sort().join(', ')
                )
                await interaction
                    .update({
    ...
    async pagination(message, description, desc = '') {
        ...
        let previousbut = new MessageButton()
            ...
            .setEmoji('<:Xerox_Arrow1:1255436702130901024>')
            .setStyle('SUCCESS')
        let nextbut = new MessageButton()
            .setCustomId('queuenext')
            .setEmoji('<:Xerox_Arrow:1255436704760729711>')
            .setStyle('SUCCESS')
