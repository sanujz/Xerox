    ...
    if (input === "add") {
      ...
      if (data)
        if (data.names.length > 0) {
          if (data.names.includes(name)) return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription(`${client.emoji.cross} | ${name} already **exist** in my custom role list!`)] })
        }
      if (!args[2]) return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription(`${client.emoji.cross} | ${message.guild.prefix}setup add ${args[1]} <role>`)] })
      ...
      if (!role) return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription(`${client.emoji.cross} | Role **not** found!`)] })
      if (role.managed) return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription(`${client.emoji.cross} | <@&${role.id}> isn't a **server** role!`)] })
     let array = ["KICK_MEMBERS", "BAN_MEMBERS", "ADMINISTRATOR", "MANAGE_CHANNELS", "MANAGE_GUILD", "MENTION_EVERYONE", "MANAGE_ROLES", "MANAGE_WEBHOOKS", "MANAGE_EVENTS", "MODERATE_MEMBERS", "MANAGE_EMOJIS_AND_STICKERS"];
      ...
      return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription(`${client.emoji.tick} | Successfully **added** <@&${role.id}> to my **custom role** list!`)] })
    } else if (input == "remove") {
      let data = await client.db?.get(`customrole_${message.guild.id}`);
      if (!data) return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription(`${client.emoji.cross} | There's **no** custom role **setup** yet!`)] })
      if (data)
        if (data?.names?.length < 1) return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription(`${client.emoji.cross} | There's **no** custom role **setup** yet!`)] })
      if (!args[1]) return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription(`${message.guild.prefix}setup add <name>`)] })
      if (!data.names.includes(args[1].toLowerCase())) return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription(`${client.emoji.cross} | role isn't **found** with the name **${args[1]}**`)] })
      ...
      return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription(`${client.emoji.tick} | Successfully **removed** ${args[1]} from my **custom role** list!`)] })
    } else if (input == "list") {
      const data = await client.db?.get(`customrole_${message.guild.id}`);
      if (!data) return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription("${client.emoji.cross} | There's **nothing** to show").setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) || client.user.displayAvatarURL() }).setFooter({ text: `Requested By ${message.author.tag}`, iconURL: `${message.author.displayAvatarURL({ dynamic: true })}` })] })
      if (data) {
        if (data.names.length < 1) return message.channel.send({ embeds: [new MessageEmbed().setDescription("<:Xerox_Success:1255432489795850280> | There's **nothing** to show").setColor('294462').setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) || client.user.displayAvatarURL() }).setFooter({ text: `Requested By ${message.author.tag}`, iconURL: `${message.author.displayAvatarURL({ dynamic: true })}` })] })
        let c = 0;
        let embed = new MessageEmbed().setColor('294462').setDescription(`${data.names.map((d, index) => `[\`${++c}\`] | **${message.guild.prefix}${d}**: <@&${data.roles[index]}> | \`${data.roles[index]}\``).join("\n")}`).setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) || client.user.displayAvatarURL() }).setFooter({ text: `Requested By ${message.author.tag}`, iconURL: `${message.author.displayAvatarURL({ dynamic: true })}` });
      ...
      return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription(`${client.emoji.tick} | Successfully **resetted** custom role module!`)] })
    } else if (input == "reqrole") {
      let data = await client.db?.get(`customrole_${message.guild.id}`);
      if (!data) return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription(`${client.emoji.cross} | There's **no** custom role **setup** yet!`)] })
      if (data)
        if (data?.names?.length < 1) return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription(`${client.emoji.cross} | There's **no** custom role **setup** yet!`)] })
      if (!args[1]) return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription(`${message.guild.prefix}setup reqrole <role>`)] })
      ...
      if (!role) return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription(`${client.emoji.cross} | Role **not** found!`)] })
      if (role.managed) return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription(`${client.emoji.cross} | <@&${role.id}> isn't a **server** role!`)] })
      if (role.position >= message.member.roles.highest.position && message.author.id !== message.guild.ownerId) return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription(`${client.emoji.cross} | <@&${role.id}> position is either **equal** or **higher** than your highest role!`)] })
      if (role.position >= message.guild.me.roles.highest.position) return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription(`${client.emoji.cross} | <@&${role.id}> position is maybe **higher** or **equal** to my highest role!`)] })
      await client.db?.set(`customrole_${message.guild.id}`, {
        roles: data.roles,
        names: data.names,
        reqrole: role.id
      });
      return message.channel.send({ embeds: [new MessageEmbed().setColor('294462').setDescription(`${client.emoji.tick} | Successfully **setup** <@&${role.id}> as custom **reqrole**`)] })
    } else if (input == "config") {
      const data = await client.db?.get(`customrole_${message.guild.id}`);
      if (!data) return message.channel.send({ embeds: [new MessageEmbed().setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) || client.user.displayAvatarURL() }).setFooter({ text: `Developed with ❤️ by ${client.user.username} Development` }).setColor('294462').setDescription(`**__Reqrole__**\n\n*Role Not set*.\n\n**__Girl__**\n\n*Role Not set*.\n\n**__Guest__**\n*Role Not set*.\n\n**__Vip__**\n\n*Role Not set*.\n\n**__Official__**\n\n*Role Not set*.`).setThumbnail(client.user.displayAvatarURL())] })
      if (data.roles.length < 1 && data.names.length < 1 && !data.reqrole)
        return message.channel.send({ embeds: [new MessageEmbed().setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) || client.user.displayAvatarURL() }).setFooter({ text: `Developed with ❤️ by ${client.user.username} Development` }).setColor('294462').setDescription(`**__Reqrole__**\n*Role Not set*.\n\n**__Girl__**\n*Role Not set*.\n\n**__Guest__**\n*Role Not set*.\n\n**__Vip__**\n*Role Not set*.\n\n**__Official__**\n*Role Not set*.`).setThumbnail(client.user.displayAvatarURL())] })
      ...
      return message.channel.send({ embeds: [new MessageEmbed().setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) || client.user.displayAvatarURL() }).setFooter({ text: `Developed with ❤️ by ${client.user.username} Development` }).setThumbnail(client.user.displayAvatarURL()).setColor('294462').setDescription(description)] })
    } else {
      return message.channel.send({ embeds: [new MessageEmbed().setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) || client.user.displayAvatarURL() }).setFooter({ text: `Developed with ❤️ by ${client.user.username} Development` }).setColor('294462').setDescription(`.\n\`${message.guild.prefix}setup add <name> <role>\`\nSetups a role with the provided name.\n\n\`${message.guild.prefix}setup remove <name>\`\nRemoves a role with the provided name.\n\n\`${message.guild.prefix}setup reqrole <role>\`\nSetups a requirement role.\n\n\`${message.guild.prefix}setup add girl <role>\`\nSetups a role for girls\n\n\`${message.guild.prefix}setup add guest <role>\`\nSetups a role for guests.\n\n\`${message.guild.prefix}setup add vip <role>\`\nSetups a role for vip members.\n\n\`${message.guild.prefix}setup add official <role>\`\nSetups a role for official mebers.\n\n\`${message.guild.prefix}setup list\`\nShows you the list of all custom roles.\n\n\`${message.guild.prefix}setup config\`\nShows you the configuration of custom roles.`)] })
    }
  }
...
async function Bitzxier(embed, client, message){
    ...
    if(Array.isArray(description)){
      ...
      embeds: [new MessageEmbed()
        ...
        .setColor('294462').setThumbnail(client.user.displayAvatarURL())
      ]
    })
