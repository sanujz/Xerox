    ...
    run: async (client, message, args) => {
        ...
        if(!data){
            ...
            const initialMessage = await message.channel.send({
                embeds: [new MessageEmbed().setColor('294462').setDescription('Configuring your server...')],
              });
            ...
            return message.channel.send({
                embeds: [
                    new MessageEmbed()
                        ...
                        .setFooter(`Note : If you wannna setup logging again use ${message.guild.prefix}logsreset & delete all existinglog channels of Xerox`, client.user.displayAvatarURL())
                ]
            });
        ...
        try {
            let category = message.guild.channels.cache.find(c => c.type === 'GUILD_CATEGORY' && c.name === 'Xerox-LOGS');

            if (!category) {
                category = await message.guild.channels.create('Xerox-LOGS', {
                    type: 'GUILD_CATEGORY',
    ...
    return message.channel.send({
        embeds: [
            new MessageEmbed()
                ...
                .setDescription('All necessary logging channels have been successfully created under the "Xerox LOGS" category.')
